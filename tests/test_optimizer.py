...
def test_registry_accumulates_and_gates(monkeypatch):
    ...
    reg = {}
    # PASS ONESTO: un pass conta solo con abbastanza dati NUOVI (NEW_DATA_MIN_S) ->
    # si avanza data_end oltre la soglia a ogni giro (3 chiamate sugli stessi dati
    # varrebbero 1 pass solo). Il passo si deriva dalla soglia, cosi' il test non si
    # rompe se la soglia cambia.
    from scripts.optimize import NEW_DATA_MIN_S
    base_end = 1_700_000_000.0
    for i in range(MIN_PASSES):
        for e in out.values():
            e["data_end"] = base_end + i * (NEW_DATA_MIN_S + 60)
